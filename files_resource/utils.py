...
class BankDataGen:
    ...
    def bankDataGen(self, shuffle_partitions_requested = 8, partitions_requested = 8, data_rows = 10000):
        ...
        fakerDataspec = (DataGenerator(self.spark, rows=data_rows, partitions=partitions_requested)
                    ...
                    .withColumn("total_transactions", "decimal", minValue=0.01, maxValue=30000, random=True)
                    .withColumn("age", "decimal", minValue=20, maxValue=100, random=True)
                    .withColumn("accounts", "decimal", minValue=1, maxValue=10, random=True)
                    .withColumn("credit_cards", "decimal", minValue=3, maxValue=15, random=True)
                    )
